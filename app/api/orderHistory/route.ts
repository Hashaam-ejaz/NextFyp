...
export async function POST(request: NextRequest) {
    ...
    if (order.buyerID.toString() !== orderHistoryData.userID.toString()) {
        return NextResponse.json({ message: "User ID does not match the order's user ID" }, { status: 400 });
      }
    //check if the orderID already exists in the OrderHistory collection
    const orderExists = await OrderHistory.findOne({ orderID: orderHistoryData.orderID });
    if (orderExists) {
        return NextResponse.json(
