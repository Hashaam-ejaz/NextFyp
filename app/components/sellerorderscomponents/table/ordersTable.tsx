...
const OrdersTable: React.FC<ordersTableProps> = ({ Orders }) => {
  ...
  useEffect(() => {
    ...
    if (currentPage === noOfPages) setIsLastPage(true);
    ...
    <>
      <div className="flex flex-col min-w-full max-w-full w-full h-auto  p-5 bg-white">
        ...
        <div className="flex flex-row w-full min-w-full max-w-full h-auto ">
          <table
            id="ordersTable"
            className="table min-w-full max-w-full w-full h-auto  border-gray-200 text-center  "
          >
                  ...
                  >
                    ...
                    <td className="py-2 px-4 border-b text-center">
                      {order.orderStatus === "unshipped" && (
                        <label className="rounded-2xl py-[2px] px-[10px] bg-[#6BAAFC] text-white text-sm font-medium leading-5 ">
                          Confirmed
                        </label>
                      )}
                      {order.orderStatus === "cancelled" && (
                        <label className="rounded-2xl py-[2px] px-[10px] bg-[#D92D20] text-white text-sm font-medium leading-5 ">
                          Cancelled
                        </label>
                      )}
                      {order.orderStatus === "shipped" && (
                        <label className="rounded-2xl py-[2px] px-[10px] bg-[#22C67F] text-white text-sm font-medium leading-5 ">
                          Shipped
                        </label>
                      )}
                      {order.orderStatus === "returned" && (
                        <label className="rounded-2xl py-[2px] px-[10px] bg-[#2222c6] text-white text-sm font-medium leading-5 ">
                          Returned
                        </label>
                      )}
                    </td>
                    <td className="py-2 px-4 border-b text-center">
                      <div className="flex">
                        {order.orderStatus !== "shipped" &&
                          ...
                          order.orderStatus !== "returned" && (
                            <button
                              className="bg-indigo-600 text-white hover:bg-indigo-900 py-1 px-2 rounded mr-2"
                              onClick={() =>
                                shipProducts(
                                  (orderIndex - productIndex),
                                  order._id
                                )
                              }
                            >
                        ...
                        <button
                          className="bg-indigo-600 text-white hover:bg-indigo-900 py-1 px-2 rounded"
        ...
        <div className="flex flex-row w-full min-w-full max-w-full h-auto justify-end  ">
          <ul className="pagination flex mt-5">
            <li className="page-item mx-1">
              <a
                href="#ordersTable"
                className={`page-link px-1 py-2 border border-[#dee2e6] ${
                  isFirstPage
                    ? "pointer-events-none text-[#6c757d] cursor-not-allowed"
                    : ""
                } font-medium text-base items-center bg-white`}
                onClick={(e) => {
                  if (isFirstPage) {
                    e.preventDefault();
                  } else {
            ...
            {numbers.map((num, index) => (
              <li
                ...
                } active:bg-[#007bff] active:text-white  active:border-[#dee2e6]`}
              >
                <a
                  href="#ordersTable"
                  className="page-link px-2 py-2 border border-[#dee2e6] font-medium text-base items-center bg-white "
